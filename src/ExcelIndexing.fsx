...
// TODO output to batches...
let outfileName (sitename:string) (ipaddr:string) : string =
    let s1 = safeName sitename
    sprintf @"G:\work\Projects\routers\output\Router Commission %s %s.xlsx" s1 (safeName ipaddr)

let outpath = outfileName @"KELDGATE/NO 3 WTW" "10.64.0.99"
...
let processRow (app:Excel.Application) (rowi:InputRow) : unit = 
    let outpath = outfileName rowi.``Colloquial name`` rowi.``Router IP Address``
...
let test02 () = 
    ...
    for (rowi:InputRow) in file.Data do
       printfn "%s, %s, %s" rowi.``SAI Number`` rowi.``Serial Number`` rowi.``AI2 Name``
...
let main () = 
    ...
    for (rowi:InputRow) in file.Data do
        ...
        | _ -> printfn "Processing %s..." rowi.``AI2 Name``
               processRow app rowi
    app.Quit()